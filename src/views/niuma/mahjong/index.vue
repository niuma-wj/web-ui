    ...
    <el-row :gutter="20">
      <el-col :span="24" :xs="24">
        ...
        <el-table v-loading="loading" :data="roomList">
          ...
          <el-table-column label="规则" align="center" width="150">
            <template slot-scope="scope">
              <span>{{ getRule(scope.row.rule) }}</span>
            </template>
          </el-table-column>
...
<script>
import { listMahjong } from "@/api/niuma/game";
export default {
  name: "Mahjong",
  data() {
    return {
  ...
  methods: {
    ...
    getList() {
      ...
      listMahjong(this.queryParams).then((response) => {
